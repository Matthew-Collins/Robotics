...
def setServoRel(Servo, Degrees, Plus):
    global ServosActive
    #print "ServosActive:", ServosActive
    #print "Setting servo"
    if ServosActive == False:
        startServos()
    pinServodRel (Servo, Degrees, Plus) # for now, simply pass on the input values
...
def startServod():
    ...
    #initString = 'sudo ' + SCRIPTPATH + '/servod --pcm --idle-timeout=20000 --p1pins="18,22" > ' + SCRIPTPATH + '/servod.log'
    initString  = 'sudo ' + SCRIPTPATH + '/servod --pcm --idle-timeout=20000 --p1pins="18,22" > ' + SCRIPTPATH + '/servod.log'
...
def pinServod(pin, degrees):
    ...
    pinString = "echo " + str(pin) + "=" + str(degrees) + " > /dev/servoblaster"
    print pinString
    os.system(pinString)
    
def pinServodRel(pin, degrees, plus):
    #print pin, degrees
    #pinString = "echo " + str(pin) + "=" + str(50+ ((90 - degrees) * 200 / 180)) + " > /dev/servoblaster"
    pinString = "echo " + str(pin) + "=" + plus + str(degrees) + " > /dev/servoblaster"
    print pinString
    os.system(pinString)
